...
LexiRacetrackProblem::LexiRacetrackProblem(char* filename, int size)
{
    size_ = size;
    ...
    s0 = new LexiRacetrackState(-1, -1, -1, -1, true, this);
    this->addState(s0);
    absorbing_ = new LexiRacetrackState(-2, -2, -2, -2, true, this);
...
void LexiRacetrackProblem::printTrack(std::ostream& os)
{
...
bool LexiRacetrackProblem::goal(mlcore::State* s, int index) const
{
...
LexiRacetrackProblem::transition(mlcore::State* s, mlcore::Action* a, int index)
{
    ...
    if (s == s0) {
        ...
        for (std::pair<int,int> start : starts_) {
            mlcore::State* next =
                new LexiRacetrackState(start.first, start.second, 0, 0, true, this);
    ...
    if (track_[rts->x()][rts->y()] == rtrack::wall) {
        ...
        mlcore::State* next =
            this->addState(new LexiRacetrackState(x + ax, y + ay, ax, ay, true, this));
...
double LexiRacetrackProblem::cost(mlcore::State* s, mlcore::Action* a, int index) const
{
...
bool LexiRacetrackProblem::applicable(mlcore::State* s, mlcore::Action* a) const
{
    LexiRacetrackState* rts = (LexiRacetrackState*) s;
    RacetrackAction* rta = (RacetrackAction*) a;
...
LexiRacetrackState* LexiRacetrackProblem::resultingState(LexiRacetrackState* rts, int ax, int ay)
{
    ...
    if (m == 0)
        return new LexiRacetrackState(x1, y1, 0, 0, isSafeLocation(x1, y1), this);

    for (int d = 0; d <= m; d++) {
        int x2 = round(x1 + (double) (d * vx) / m);
        int y2 = round(y1 + (double) (d * vy) / m);
        if (track_[x2][y2] == rtrack::wall) {
            return new LexiRacetrackState(x2, y2, 0, 0, true, this);
        }
        if (track_[x2][y2] == rtrack::goal) {
            return new LexiRacetrackState(x2, y2, vx, vy, true, this);
        }
    }
    return new LexiRacetrackState(x1 + vx, y1 + vy, vx, vy,
...
void LexiRacetrackProblem::addSuccessors(int idAction,
                                          LexiRacetrackState* source,
                                          LexiRacetrackState* dest,
                                          double prob)
{
    std::vector<mlcore::SuccessorsList>* allSuccessors = source->allSuccessors();
    if (useSafety_ && !source->safe() && dest->safe()) {
        LexiRacetrackState* dest2 =
            new LexiRacetrackState(dest->x(), dest->y(), dest->vx(), dest->vy(), false, this);
