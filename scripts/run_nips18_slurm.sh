...
output_dir="/home/lpineda/results_nips18/brtdp_1ksims"
...
for ((ip = 0; ip < ${#problems[@]}; ip++)); do
  ...
  # SOFT-FLARES (best racetrack)
  sbatch ${swarm_flags} --output=${output_dir}/${problem_str}_"soft-flares_3_traj_logistic".txt \
    run_testsolver.sh "$problem" $nsims $reps $verbosity $min_time $max_time "$other_flags" \
    "soft-flares --labelf=logistic --dist=traj --horizon=3 --alpha=$alpha"
