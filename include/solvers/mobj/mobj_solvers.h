...
/**
 * Computes the Q-value of a state-action pair for the given multi-objective MDP.
...
/**
 * Samples a trial of the greedy policy implied by state->bestAction() and returns
 * the accumulated discounted cost (using problem->gamma()) over all cost functions.
 * The trial starts at the given state s.
 *
 * @param problem The multi-objective problem that defines the transition function.
