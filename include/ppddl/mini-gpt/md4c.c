...
/* MD4_memcpy and MD4_memset are just memcpy and memset. */
#define MD4_memcpy(output, input, len ) \
  memcpy( output, input, len * sizeof( char ) );

#define MD4_memset(output, value, len ) \
  memset( output, value, len * sizeof( char ) );
