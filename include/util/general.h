...
/**
 * Computes the next combination (no repetitions) of numbers
 * from 0 to (n-1) chosen k at a time.
 * The method returns true unless no more combinations are possible.
 *
 * This method modifies the given vector in place.
 *
 * For instance, calling nextComb(<0,2>, 3, 2) stores the
 * vector <1,0> and returns true, while calling
 * nextComb(<1,2>, 3, 2) returns false.
 */
bool nextComb(std::vector<int>& comb, int n, int k);

/**
 * Computes the next combination (with repetitions) of n elements
 * with k options for each.
 * The method returns true unless no more combinations are possible.
 *
 * This method modifies the given vector in place.
 *
 * For instance, calling nextComb(<1,0>, 2) stores the
 * vector <1,1> and returns true, while calling nextComb(<1,1>, 2)
 * returns false.
 */
